...
mut_counts=Read10X_h5('Data_name.h5', use.names = TRUE, unique.features = TRUE)
...
mut_obj <- CreateSeuratObject(counts = mut_counts, project = "MUT", min.cells = 3, min.features = 200)
mut_obj.totalCell=as.integer(dim(mut_counts)[2])
...
res_val="res_0.02"
all.clean <- FindClusters(all.clean, verbose = FALSE,resolution = 0.02)
head(all.clean[[]])
...
scrna=all.clean
save(scrna,file = paste('MUT_analysis.RData',sep=''))
rm(scrna)
...
load("~/Folder with data/MUT/MUT_analysis.RData")
merge=scrna # 
rm(scrna)
...
pdf(file = paste('Heatmaps_and_DotPlots_MUT.pdf',sep=''),height = 13,width = 20)
